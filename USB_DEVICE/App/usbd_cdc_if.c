...
extern BUF_RING_8 ring_buf_adc_usb;
/* 判断是否已经发送完成 */
extern int8_t sig_sensor_data_usb_send;
...
static int8_t CDC_Receive_FS(uint8_t epnum, uint8_t* pbuf, uint32_t *Len);
...
uint8_t CDC_Transmit_FS(uint8_t epnum, uint8_t* Buf, uint16_t Len)
{
  ...
  if (hcdc->TxState[(epnum&0x0F)-1] != 0){
    return USBD_BUSY;
  }
